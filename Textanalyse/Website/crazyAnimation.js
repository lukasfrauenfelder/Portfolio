...
function highlightWords() {
  ...
  Verbs.forEach((verb) => {
    ...
    verb.style.WebkitTextStroke = "1px #000";
    console.log(verb.style);
  });
  ...
  for (let i = 0; i < 10; i++) {
    ...
    Verbs[auswahl].style.WebkitTextStroke = "1px #8CB1FA";
    Verbs[auswahl].style.textShadow = make_text_shadow(b);
    ("3px 3px 0px #8CB1FA, 6px 6px 0px #8CB1FA, 9px 9px 0px #8CB1FA"); // Correct syntax

    // Select the rest of the spans
    Verbs.forEach((verb, index) => {
      if (!selectedIndices.includes(index)) {
        verb.classList.add("animate-stroke");
      }
      // Revert to original stroke after the animation
      setTimeout(() => {
        verb.classList.remove("animate-stroke");
